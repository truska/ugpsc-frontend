...
mem_page_header('UGPSC Members | Renew Membership', ['active' => 'dashboard']);
?>
<div class="container" style="max-width:860px;">
  <div class="mem-card p-4 p-lg-5">
    <div class="d-flex justify-content-between align-items-start flex-wrap gap-2 mb-3">
      <div>
        <h1 class="display-font h3 mb-1">Renew Membership</h1>
        <p class="text-secondary mb-0">Complete a card payment to renew membership for one year.</p>
      </div>
      <span class="badge text-bg-secondary text-uppercase">Logged In Renewal</span>
    </div>
...
(() => {
  ...
  const createIntent = () => {
    ...
    formData.append('flow', 'renew_logged_in');
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (paymentIntent && paymentIntent.status === 'succeeded') {
      ...
      finalizeData.append('flow', 'renew_logged_in');
